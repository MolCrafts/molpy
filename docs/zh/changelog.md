...
- **力场模型完全迁移到 molrs 中；`molpy.potential` 变为外观模块。** `molpy.core.forcefield` 是原生 molrs `ForceField` / Style / Type 层级结构的轻量重导出（外加 `ForceField` 别名和命名的专用 Style 类）。删除了 `potential/` 下的 Python 内核和能量计算代码。`molpy.potential[.bond|.angle|…]` 重导出基于 molrs 的 Style 类和 `Potentials`，用户无需直接导入 molrs。能量/力通过 `ff.to_potentials(frame).calc_energy(frame)` / `.calc_forces(frame)` 计算；`optimize.ForceFieldPotential` 包装了此流程（移除了 `potential_wrappers` 中各内核的包装器）。I/O 格式器改为按样式/内核名称分发（不再需要各内核的 `Type` 类），`def_type` 参数改为仅关键字参数。
