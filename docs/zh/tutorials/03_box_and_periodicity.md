...
盒子以 `frame.simbox` 的形式附加到 Frame 上，而不是存在 metadata 里。这是将模拟单元格与分子数据关联的标准方式。
...
# I/O 读取器会自动设置 frame.simbox
frame = mp.io.read_lammps_data("system.data", atom_style="full").frame
print(frame.simbox.lengths)   # 来自 data 文件头
```

所有计算算子（MSD、RDF 等）都会从 `frame.simbox` 读取盒子信息。
