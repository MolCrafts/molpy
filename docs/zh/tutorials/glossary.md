...
**Reaction（反应）**
:   一条反应 SMARTS。它匹配反应物模式，成键与断键，并删除只出现在左侧、不出现在右侧的原子（离去基团）。全部化学都写在这里。参见[组装](../user-guide/02_assembly.md)。

**GraphAssembler（图组装器）**
:   把分子粘贴进同一个世界，在 `Selector` 指定的位置施加 `Reaction`，并修复每根新键附近的力场类型。`PolymerBuilder` 就是一个 `GraphAssembler`，只是它额外拥有一个单体库并能读 CGSmiles。

**Site（位点）**
:   原子上的一个名字（`fields.SITE`），表示该原子可以参与反应。位点没有方向、也没有角色——线型链、支化点和成环之间的差别，只在于单体带几个位点、以及拓扑如何把它们配对。
