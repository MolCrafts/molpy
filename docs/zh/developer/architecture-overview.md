...
| `builder` | 系统组装：一个 `GraphAssembler` 内核 + `Selector` 族、虚拟位点、AmberTools 集成 |
| `typifier` | 图分型：一个 `Typifier` 契约（`MolGraph -> MolGraph`）、molrs OPLS-AA/MMFF re-export，以及 CL&P 等 MolPy 侧叠加 |
...
`core` 没有任何上层依赖，其他模块都构建在 `core` 之上。`compute`、`io`、`engine` 操作表格层（`Frame`/`Block`）；`parser`、`builder`、`typifier` 操作图结构层（`Atomistic`）。`wrapper` 和 `adapter` 在最外层，不允许外部类型泄漏进 `core`。
