...
这片邻域有多大，不是 typifier 上的可调旋钮。`GraphAssembler` 需要知道分型器的 `reach`——它要看到一个原子周围几根键，才能叫出这个原子的类型——而这一个数字同时定下了操作的两个半径。`AffectedRegion.around` 围绕每根新键取出一个半径为 `2 x reach` 的球，但只把内层 `reach` 那一壳写回去：真正改变了环境的正是这些原子，而外壳存在的唯一理由，是给它们提供一个正确的环境去匹配。内壳之外的原子本来就是对的，因此不去动它们。
