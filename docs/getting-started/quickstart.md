...
# get_topo perceives angles/dihedrals in place (returns self for chaining)
...
Load the bundled `tip3p.xml` and spend its types on the template.

> **Note:** nothing here has to *decide* a type. A rigid TIP3P water is three
> named sites, so this is `ForceFieldParams` — the component that annotates
> pair and bonded terms from types already on the graph — and not a `Typifier`,
> which is the thing that decides types in the first place. Reach for a
> typifier (`OPLSAATypifier`, `AmberToolsTypifier`, …) when the types are the
> unknown.
