...
LAMMPS `*.ff` includes are **read and written by molrs**
(`molrs.ff.read_lammps_forcefield` / `write_lammps_forcefield`). Molpy's
`mp.io.read_lammps_forcefield`, `write_lammps_forcefield`, and
`LAMMPSForceFieldWriter` are thin facades. Params live in molrs units
