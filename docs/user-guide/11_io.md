...
Readers turn files into `Frame`s; writers turn `Frame`s back into files — one pattern across every format. To add support for a **new** format, see [Developer Guide: I/O Formats](../developer/extending-io.md).
