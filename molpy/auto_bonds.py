...
def auto_bonds(mpObj, auto_style = "LAMMPS-INTERFACE"):
    if auto_style == "LAMMPS-INTERFACE":
        auto_bonds_LAMMPS_INTEFACE(mpObj)
    else:
        raise NotImplementedError(f"The auto bond style of {aut_style} is not implemented!\n");

def auto_bonds_LAMMPS_INTEFACE(mpObj, scale_factor : float = 0.9):
