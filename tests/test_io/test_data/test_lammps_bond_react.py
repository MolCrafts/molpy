...
def test_system_writer_matches_golden(
    ...
    for name in sorted(produced):
        if name == "sys.ff":
            produced_lines = [li for li in produced[name].splitlines() if li.strip()]
            golden_lines = [li for li in golden[name].splitlines() if li.strip()]
            assert sorted(produced_lines) == sorted(golden_lines), (
