...
class TestSMARTS:
    ...
    def test_uniqueness(self, rule_match, test_data_path):
        mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/uniqueness_test.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(mol2["atoms"])
        }
    ...
    def test_ringness(self, rule_match, test_data_path):
        ring_mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/ring.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(ring_mol2["atoms"])
        }

        rule_match(ring_mol2, typemap, "[#6]1[#6][#6][#6][#6][#6]1", True)

        not_ring_mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/not_ring.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(not_ring_mol2["atoms"])
        }
    ...
    def test_fused_ring(self, smarts_parser, test_data_path):
        mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/fused.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(mol2["atoms"])
    ...
    def test_ring_count(self, smarts_parser, test_data_path):
        ...
        # One ring
        ring = mp.io.read_mol2(
            test_data_path / "data/mol2/ring.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(ring["atoms"])
        }
    ...
    def test_pf6(self, rule_match_count, test_data_path):
        mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/pf6.mol2"
        ).frame.to_struct()
        typemap = {
            i: {"whitelist": set(), "blacklist": set(), "atomtype": None}
            for i, _ in enumerate(mol2["atoms"])
        }
        typemap[0]["whitelist"].add("P")
        # typemap[1]["whitelist"].add("F1")
        # typemap[2]["whitelist"].add("F1")
        # typemap[3]["whitelist"].add("F2")
        # typemap[4]["whitelist"].add("F2")

        checks = {
            # "P": 1,
            "FP([!%F1])([!%F1])([!%F1])([!%F1])F": 2,
            # "FP([!%F1;!%F2])([!%F1;!%F2])([%F1])([%F1])F": 4,
            # "FP([%F1])([%F1])([%F2])([%F2])F": 2,
        }
    ...
    def test_hexa_coordinated(self, test_data_path):
        ff = mp.io.read_xml_forcefield(test_data_path / "forcefield/xml/pf6.xml").forcefield
        mol2 = mp.io.read_mol2(
            test_data_path / "data/mol2/pf6.mol2"
        ).frame.to_struct()
