"""Architecture checks for the AmberTools builder unit-test package."""

import ast
from pathlib import Path


class TestAmberToolsTestLayout:
    SOURCE = (
    ...
    def test_test_modules_mirror_source_modules(self):
        source_modules = {
            path.stem for path in self.SOURCE.glob("*.py") if path.name != "__init__.py"
        }
        test_modules = {
    ...
    def test_each_production_class_has_its_own_test_class(self):
        for source in self.SOURCE.glob("*.py"):
            ...
            test_file = self.TESTS / f"test_{source.stem}.py"
            test_classes = {
                node.name
