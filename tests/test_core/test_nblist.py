...
class TestNaiveNblist:
    ...
    @pytest.mark.parametrize(
        "cutoff, self_interaction",
        [(rc, self_interaction) for rc in [7] for self_interaction in [False, True]],
    )
    def test_orth_space(self, ase_orth_tests, cutoff, self_interaction):
        ...
        for atoms in ase_orth_tests:
            ...
            # )
            # npt.assert_equal(actual_dist, expected_dist, err_msg=f"{}")
            expected_mapping = np.unique(np.stack([idx_i, idx_j], axis=-1), axis=0)
            npt.assert_equal(mapping, expected_mapping)