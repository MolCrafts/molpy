...
from molpy.external import TLeapWrapper
...
def test_tleap_wrapper_run_script():
    ...
    with (
        ...
        proc = wrapper.run_script(script_text=script_text, script_name="test.in")

        # Verify script was written
        mock_write.assert_called_once_with(script_text)

        # Verify tleap was called with correct args
...
def test_tleap_wrapper_run_script_default_name():
    ...
    with (
        ...
        # Verify default script name was used
        call_args = mock_run.call_args[0][0]
