...
SCENARIO("Test Absolute Coordinates") {
  ...
  Box obox(2.0, 2.0, 2.0);                // in original
  Box tbox(2.0, 2.0, 2.0, 1.0, 1.0, 1.0); // triclinic

  GIVEN("a vec3f") {
    vec3f frac_point = {0.5, 0.25, 0.75};

    THEN("test absolute") {
      REQUIRE(
          xt::allclose(obox.makeAbsolute(frac_point), vec3f{1.0, 0.5, 1.5}));
      REQUIRE(xt::allclose(tbox.makeAbsolute(frac_point), vec3f{1., 1., 2.25}));

      // test out of place operation
      REQUIRE(xt::allclose(frac_point, vec3f{0.5, 0.25, 0.75}));
    }
  }

  GIVEN("a xt::xarray") {

    xt::xarray<float> frac_points = {
        {0.5, 0.25, 0.75}, {0, 0, 0}, {0.5, 0.5, 0.5}};

    THEN("test absolute") {

      REQUIRE(xt::allclose(obox.makeAbsolute(frac_points),
                           xt::xarray<float>{{1.0, 0.5, 1.5}, {}, {}}));
      REQUIRE(xt::allclose(tbox.makeAbsolute(frac_points), vec3f{1., 1., 1.5}));
    }
  }
...
//         {8, -2, -2}, {0.25, 0.75, 0.5}, {-2.1, 3.7, 4.7}};
...
//   GIVEN("a xt::xarray") {
//     xt::xarray<float> f_point = {{0.5, 0.25, 0.75}, {0, 0, 0}, {0.5, 0.5,
//     0.5}}; xt::xarray<float> point = {{0, -0.5, 0.5}, {-1, -1, -1}, {0, 0,
//     0}};
