...
class GroReader(DataReader):
    ...
    def _parse_atom_line(self, line: str) -> dict | None:
        ...
        try:
            ...
            if len(line) >= 68:
                try:
                    atom_data['vx'] = float(line[44:52].strip()) if line[44:52].strip() else 0.0
                    atom_data['vy'] = float(line[52:60].strip()) if line[52:60].strip() else 0.0
                    atom_data['vz'] = float(line[60:68].strip()) if line[60:68].strip() else 0.0
                except ValueError:
    ...
    def read(self, frame):
        ...
        try:
            ...
            print(f"Warning: Error reading GRO file {self._file}: {e}")
            # Create empty frame
...
class GroWriter(DataWriter):
    ...
    def _format_atom_line(self, res_num: int, res_name: str, atom_name: str, 
                         atom_num: int, x: float, y: float, z: float,
                         vx: float | None = None, vy: float | None = None, vz: float | None = None) -> str:
    ...
    def _get_atom_data_at_index(self, atoms_dataset, index: int) -> dict:
        ...
        for var_name in atoms_dataset.data_vars:
            ...
            if var_name == "xyz":
                ...
                vel_dims = [d for d in values.dims if str(d).startswith('dim_velocity_') and not str(d).endswith('_1')]
                if vel_dims:
                    vel_dim = vel_dims[0]
    ...
    def write(self, frame):
        ...
        with open(self._path, "w") as f:
            # Write title line
            title = frame.get("name", "Generated by molpy")
            ...
            if n_atoms > 0:
                atoms = frame["atoms"]
                has_velocity = ("vx" in atoms.data_vars and 
                              "vy" in atoms.data_vars and 
                              "vz" in atoms.data_vars)
                ...
                for i in range(n_atoms):
                    ...
                    atom_num = atom_data.get("atomic_number", i + 1)
                    ...
                    # Velocity (optional)
                    vx = atom_data.get("vx") if has_velocity else None
                    vy = atom_data.get("vy") if has_velocity else None
                    vz = atom_data.get("vz") if has_velocity else None
