...
def create_vdw_linear_placer(buffer: float = 0.0) -> Placer:
    """
    Create a placer with VdW separation and linear orientation.

    NOTE: VdW radii give large separations (~3-4 Å) suitable for non-bonded contacts.
    For bonded assembly, use create_covalent_linear_placer() instead.

    Args:
        buffer: Additional buffer distance in Angstroms (default: 0.0)

    Returns:
        Placer instance configured for VdW-based linear assembly
    """
    separator = VdWSeparator(buffer=buffer)
    orienter = LinearOrienter()
    return Placer(separator, orienter)


def create_covalent_linear_placer(buffer: float = 0.0) -> Placer:
    """
    Create a placer with covalent bond length separation and linear orientation.

    Uses realistic bond lengths (e.g., C-C: 1.54 Å, C-O: 1.43 Å) for bonded assembly.
    This is the recommended placer for polymer assembly.

    Args:
        buffer: Additional buffer distance in Angstroms (default: 0.0)
                Can be negative to slightly compress the bond

    Returns:
        Placer instance configured for covalent bond-based linear assembly
    """
    separator = CovalentSeparator(buffer=buffer)
    orienter = LinearOrienter()
    return Placer(separator, orienter)
