"""Agent-only polymer building Tool classes.

These dataclasses wrap the parser, adapter, builder, and reacter modules
into single-call workflows for AI-agent orchestration. They are
infrastructure for agent runtimes, not part of the user-facing API:
none of them is re-exported from ``molpy.builder`` or
``molpy.builder.polymer``. Human users should reach for the
``polymer()`` / ``polymer_system()`` entry functions in
``molpy.builder.polymer.dsl`` instead.
"""
