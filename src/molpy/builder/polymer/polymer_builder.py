...
from .port_utils import get_all_port_info, set_port_metadata, PortInfo, get_port_metadata
...
class PolymerBuilder:
    ...
    def _build_from_graph(
        ...
        Strategy (方案B - Graph Building + Delayed Merging):
    ...
    def _dfs_connect(
        ...
        for neighbor_id, bond_order in neighbors:
            ...
            current_group = [nid for nid, mon in monomers.items() if mon is current_monomer]
            
            # Find all nodes that currently point to neighbor_monomer
            neighbor_group = [nid for nid, mon in monomers.items() if mon is neighbor_monomer]
    ...
    def _connect_monomers(
        ...
        left_port_name, left_port_idx, right_port_name, right_port_idx, _ = self.connector.select_ports(
            left,
            right,
            left_ports,
            right_ports,
        ...
        # Flatten the lists of ports into a single dict
